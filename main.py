...
class WindowClass(QMainWindow, Ui_Dialog):
    def __init__(self):
        ...
        h1_list = [self.label_1, self.label_2, self.label_3]
    ...
    def dropEvent(self, e):
        if e.mimeData().hasUrls:
            ...
            if is_xlsx(dir_path):
                ...
                print(self.isSetInput, self.isSetOutput)
                if not self.isSetInput:
                    self.input_file_path = dir_path
                    self.lineEdit_excel_1.setText(f"{self.input_file_path}")
                    self.textEdit_log.setText(f"{self.textEdit_log.toPlainText()}쿠팡 엑셀 위치: {self.input_file_path}\n")
                    self.isSetInput = True
                elif not self.isSetOutput:
                    self.output_file_path = dir_path
                    self.lineEdit_excel_2.setText(f"{self.output_file_path}")
                    self.textEdit_log.setText(f"{self.textEdit_log.toPlainText()}리포트 엑셀 위치: {self.output_file_path}\n")
    ...
    def finish_task(self, rst):
        ...
        if rst:
            ...
            self.textEdit_log.setText(f"{self.textEdit_log.toPlainText()}{self.input_file_path}sheet가 여러개 입니다. 실행 종료\n")
        self.isSetInput = False
        self.isSetOutput = False


    def auto_report(self, data_xls_name, report_template_xls_name):
    ...
    def push_execute(self):
        self.btn_execute.setEnabled(False)
        rst = self.auto_report(self.input_file_path, self.output_file_path)
